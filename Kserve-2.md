...
tags:
  - hover-notes
  - udemy
hovernotes-id: doc_4a942672-ce3b-4105-9249-344722c97d84
---
...
flowchart LR
    ...
    Controller -->|Triggers| HPA[HPA & Deployment]
```
...
```mermaid
sequenceDiagram
    participant User as User (kubectl apply)
...
```mermaid
sequenceDiagram
    participant Pipeline as CI/CD / CI/CD-CT Pipeline
...
flowchart TD
    ...
    subgraph Domain Setup
        ...
        Attach --> Domain[Create Domain with Role & Networking]
    end
...
```mermaid
flowchart TD
    A[Stop & Delete Spaces/Apps] --> B[Delete User Profiles]
    B --> C[Delete Domain]
```
...
### Kubeflow Pipelines (KPF)

- The tutorial will cover how to write Kubeflow Pipelines
    - Starting with a "Hello World" example
    - Moving into a deep dive of real-world KPF pipelines

### Real World KPF Pipeline

- Transitioning from basic examples to a real-world model to gain a deeper understanding of Kubeflow Pipelines
- Hands-on practice via a GitHub repository containing all necessary commands and notes

### Machine Learning Lifecycle

- To understand Kubeflow, it is helpful to first understand the Machine Learning Lifecycle
- This lifecycle is an iterative process, much like the traditional Software Development Lifecycle (SDLC)
- **[SDLC Comparison]** Just as software requires stages to build and ship an application, ML requires a structured flow to manage its complexity

#### Software Development Lifecycle (SDLC)

- A series of stages used to develop and deploy software applications
- Common stages include:
    - Planning
    - Requirement gathering and analysis
    - High-level and low-level design
    - Implementation (development and writing build scripts)
    - Testing
    - Deployment
    - Constant monitoring
...
- A specialized iterative process for building and maintaining machine learning models
...
flowchart TD
    ...
    D --> E["Model Training"]
    E --> F["Model Evaluation and Tuning"]
    F --> G["Model Deployment"]
    G --> H["Model Monitoring and ..."]
    H --> A
```

### The Role of DevOps

- Emerged because the SDLC is a highly iterative process
- **[The Problem]** Manually repeating the entire cycle (implementation, testing, deployment, monitoring) for every change is time-consuming for developers
- **[The Goal]** To unify development and operations and eliminate manual operational effort
- DevOps engineers introduce specific tools at different stages to automate and streamline the lifecycle:
    - **Build Stage**: Tools like `Maven` or `Gradle` (depending on the programming language)
    - **Deployment Stage**: Continuous Delivery (CD) tools
    - **Testing Stage**: Continuous Testing tools
    - **Monitoring Stage**: Observability platforms

### Impact of DevOps

- DevOps significantly reduces production time by automating manual interventions
    - Example: Production time might drop from 3 months to 1 month

### ML Lifecycle: Problem Definition

- Just as traditional software requires an SDLC, model development requires its own structured lifecycle
- **[Problem Definition]** The initial stage where the specific problem statement is established
    - This involves determining what kind of model is needed
    - Examples include predicting if a patient has diabetes or performing a 4-test classification
- Once the problem is defined, the process moves toward identifying exact machine learning algorithms and developing the model

### Machine Learning Lifecycle: Data and Training

- **[Data Collection]** The most critical stage for any ML or AI model
    - Accuracy is directly tied to the amount and relevance of data
    - Example: A weather prediction model may require 30 to 40 years of historical data to ensure accuracy
- **[Data Analysis and Serialization]** Necessary because collected data is often in an unorganized format
- **[Feature Engineering and Selection]** An iterative development process
    - Developers continuously work on different features rather than developing the model in a single pass
- **[Model Training]** The stage where the model is actually trained using the collected data
    - This involves identifying the specific algorithm required, such as `Random Forest` or `Isolation Forest`
    - Once the algorithm is chosen, the training process begins

### The Importance of Model Accuracy

- Accuracy levels can vary significantly (e.g., 60%, 80%, or 99.9%)
- **[Why accuracy matters]** In high-stakes environments like healthcare, low accuracy is unacceptable
    - Example: An X-ray analysis model with only 60% accuracy might incorrectly report a fracture when none exists (false positive)
- Accuracy is determined by three primary factors:
    - The choice of machine learning algorithm
    - How well the model is trained
    - The quality and quantity of data provided (e.g., hundreds of thousands of images for a robust model)

### Machine Learning Lifecycle: Deployment and Monitoring

- **[Evaluation and Tuning]** Once a model is trained, it must be constantly evaluated and tuned to ensure performance meets requirements
- **[Deployment]** The model is packaged with software and made accessible to users, often via a website or as part of an existing application
- **[Monitoring and Retraining]** Models must be continuously monitored because their efficiency tends to degrade over time
    - **[Why retraining is necessary]** As new data enters the system, a model that was once 99% accurate might drop to 90% efficiency
    - This necessitates a loop of retraining with new data, followed by re-evaluation, re-tuning, and re-deployment

### The Challenge of Production Deployment

- The Machine Learning Lifecycle is significantly more iterative than the traditional Software Development Lifecycle (SDLC)
- **[The "Laptop" Problem]** Many machine learning engineers struggle to move models into actual production environments
    - Studies suggest that approximately 80% of models remain "buried" on engineers' laptops and never reach production

### The Emergence of MLOps

- **[The Problem]** The Machine Learning lifecycle is inherently iterative; model efficiency tends to decrease over time due to:
    - Constant influx of new data
    - Changing feature requirements
- **[The Solution]** MLOps (Machine Learning Operations) was created as an extension of DevOps to handle these iterations
    - It applies DevOps principles specifically to the Machine Learning lifecycle
    - **[How it works]** MLOps introduces tools and components to automate stages where high manual effort or complex operations are required
- **[Key focus areas for MLOps automation]**
    - Data loading
    - Data analysis
    - Model training
- **[Goal]** Much like CI/CD (Continuous Integration/Continuous Deployment) in DevOps, MLOps aims to significantly reduce the time and manual effort required to maintain and update models

### The Scope of MLOps

- **[Beyond CI/CD]** While MLOps incorporates CI/CD principles, it extends further to encompass the full operational lifecycle, much like how DevOps goes beyond simple deployment to include constant application observation.
- **[Comprehensive Operations]** To effectively reduce operational effort, MLOps must integrate:
    - CI/CD specifically tailored for ML workflows
    - Continuous monitoring of model performance
    - Continuous tuning of models
- **[The Bridge]** Ultimately, MLOps acts as the unifying bridge between machine learning development and operational execution.

### Implementing MLOps

- **[The Two Approaches]** There are two main ways to handle MLOps in a company:
    - **Individual Tools**: Adding a specific, standalone tool at every individual stage of the Machine Learning lifecycle
    - **Standalone Platforms**: Using comprehensive, all-in-one platforms that manage the entire lifecycle
- **[MLOps Platforms]**
    - **Kubeflow**: A sophisticated, standalone platform (the focus of this tutorial)
    - **MLflow**: Another platform used to solve similar operational problems, though it is out of context for this specific session

### Kubeflow Components

- **[Nature of Kubeflow]** It is not a single tool or project, but a collection of various components
    - Each component is a project designed to solve specific operational aspects of machine learning
- **[Key Components and Their Uses]**
        - **Kubeflow Pipelines**: Used for tasks like loading data
        - **KServe**: Handles model deployment and exposing models to the external world
        - **Kubeflow Notebooks**: Useful for execution
                - Primarily used by ML engineers to write Python scripts
                - Allows for step-by-step evaluation and execution of program parts

### Additional Kubeflow Components

- **Katib**: Used for hyperparameter tuning of the model
- **Model Registry**: A dedicated registry for models, analogous to an application or container registry
- **Kubeflow Dashboard**: Serves as the unified User Interface (UI) for the entire Kubeflow project
    - **[Unified Access]** It allows users to access various underlying projects and services from a single location
    - **[Analogy]** Much like the AWS dashboard provides a single interface to access multiple services (e.g., EC2, S3), the Kubeflow dashboard provides access to all its constituent components

### Kubeflow in the Machine Learning Lifecycle

- **[Mapping Components to Stages]**
    - **Data Analysis, Feature Engineering, and Selection**
        - Can be performed using Kubeflow Notebooks (or Jupyter Notebooks)
        - Can also be managed via Kubeflow Pipelines (KPF)
- **[Mapping Components to Stages (Continued)]**
    - **Model Training**
        - Can be managed using the **Training Operator**
    - **Model Evaluation**
        - **Kubeflow Pipelines (KPF)** can be used to evaluate model efficiency
    - **Model Deployment**
        - **KServe** is used for deploying models
    - **Model Monitoring**
        - **KServe** can be integrated with observability platforms
            - Examples include **Prometheus** and **Grafana**
- **[Learning Strategy]**
    - **Approach**: Focus on learning individual components one by one rather than trying to master the entire Kubeflow ecosystem at once
    - **[Analogy]** This is similar to learning DevOps, where you learn specific pieces like Build, Deployment, CI, and CD separately

### Learning Kubeflow

- **[Strategy]** Learn component by component rather than trying to master the entire ecosystem at once
    - This prevents feeling overwhelmed by the complexity of MLOps
- **[Flexibility]** Kubeflow allows for modular usage
    - You can install the entire suite or selectively use specific components (e.g., using KServe but skipping the Training Operator) depending on organizational needs

### Kubeflow Pipelines (KPF)

- **The most important component**
    - Functions similarly to CI/CD in the DevOps world
- **[Capabilities]** KPF orchestrates the various stages of the ML lifecycle within a single pipeline:
        - Data loading
        - Model training
        - Model evaluation
...
```mermaid
flowchart LR
    KPF["Kubeflow Pipelines (KPF)"] --> Loading["Data Loading"]
    KPF --> Training["Training"]
    KPF --> Evaluation["Evaluation"]
    style KPF fill:#f9f,stroke:#333,stroke-width:2px
```

### Installing Kubeflow

- **[Modular Installation]** You can choose between two primary methods:
    - **Full Installation**: Installing the entire Kubeflow suite at once.
    - **Standalone Components**: Installing only specific projects needed (e.g., Kubeflow Pipelines, Katib, or Model Registry).
- **[Using Official Documentation]** Use the official Kubeflow documentation rather than searching external sites for the most accurate and up-to-date installation steps.
    - To install a specific component:

        1. Navigate to **Kubeflow Projects**
        2. Select the desired project (e.g., Katib, Model Registry, Kubeflow Trainer, or Kubeflow Pipelines)
        3. Go to the **Operator Guide**
        4. Select the **Installation** section

- **[Local Installation]**
    - **MiniKF**: A solution designed to help install Kubeflow on a laptop, similar to how Minikube works for Kubernetes.

### Kubernetes Requirement for Kubeflow

- **[Dependency]** A Kubernetes cluster is required to run Kubeflow
    - Kubeflow is tightly coupled with Kubernetes
    - The core purpose of Kubeflow is to implement the machine learning lifecycle on a Kubernetes cluster
- **[Execution Model]** Kubeflow stages run as containerized units
    - Each stage of a pipeline (e.g., data loading, training, evaluation) runs as containers or pods within the Kubernetes cluster

```mermaid
flowchart TD
    K[Kubeflow Pipeline Stage] -->|Runs as| C[Container/Pod]
    C -->|Managed by| K8s[Kubernetes Cluster]
```

### Local Installation Setup

- **[Prerequisites]** Because Kubeflow is tightly coupled with Kubernetes, a cluster is required to run the pipeline stages (which run as containers or pods)
- **Step 1: Install Docker Desktop**
    - Required to run a Kubernetes cluster in minutes on a laptop
    - Works across Windows, Linux, and macOS
- **Step 2: Install KIND (Kubernetes in Docker)**
    - Used to set up a KIND k8s cluster to host the pipelines
- **[Resources]** A GitHub Gist is provided containing the complete installation guide and the specific pipelines to be executed

#### Installation Roadmap (from Gist)

1. Install Docker Desktop
2. Install KIND
3. Install KPF in Kubernetes
4. Install Python kfp package

```mermaid
flowchart TD
    A[Laptop] --> B[Install Docker Desktop]
    B --> C[Install KIND]
    C --> D[Create KIND Cluster]
    D --> E[Install KPF in Kubernetes]
    E --> F[Install Python kfp package]
```

### Creating a KIND Cluster for KFP

- **[Command]** To create a specific cluster for the demo, use the following command:

```bash
kind create cluster --name kfp-demo-yt
```

- **[Verification]** Once the cluster is created, verify that `kubectl` is connected to the new cluster by checking the nodes:
...
- **[Mechanism]** Because Kubeflow is tightly coupled with Kubernetes, deployment involves installing Custom Resource Definitions (CRDs) that allow Kubernetes to understand KFP-specific resources.
- **[Deployment Steps]** The deployment process typically involves:

    1. Exporting the specific Kubeflow Pipeline version as an environment variable
    2. Applying the configuration files to the cluster

Example of the deployment commands:

```bash
export PIPELINE_VERSION=2.4.0

# Install cluster-scoped resources (CRDs)
kubectl apply -k github.com/kubeflow/pipelines/manifests/customize/cluster-scoped-resources?ref=$PIPELINE_VERSION

# Wait for the CRDs to be established
kubectl wait --for condition=established --timeout=60s crd/applications.app.k8s.io

# Install environment-specific resources
kubectl apply -k github.com/kubeflow/pipelines/manifests/customize/env/dev?ref=$PIPELINE_VERSION
```

### The Kubeflow Pipelines (KFP) Workflow

- **[Workflow]** The process follows a transition from high-level Python code to low-level Kubernetes orchestration:

    1. **Write**: ML engineers write a Python script defining the pipeline stages.
    2. **Compile**: The `kfp` Python package (installed via `pip`) compiles that script into a YAML file.
    3. **Submit**: The YAML file is submitted to the Kubernetes cluster.
    4. **Execute**: Kubernetes runs the pipeline by triggering containers or pods for each individual stage.

- **[Example]** In a typical three-stage pipeline (e.g., Data Loading $\rightarrow$ Training $\rightarrow$ Evaluation), Kubernetes will trigger separate containers or pods for each of those steps.
- **[Cloud Native Nature]** This approach is entirely cloud-native, behaving similarly to how GitHub Actions creates virtual machines for different stages of a workflow.

```mermaid
flowchart LR
    A[Python Script] -->|`kfp` package compiles| B[YAML File]
    B -->|Submitted to| C[Kubernetes Cluster]
    C --> D[Pod/Container: Data Loading]
    C --> E[Pod/Container: Training]
    C --> F[Pod/Container: Evaluation]
```

### Kubeflow Pipelines Deployment Details

- **[Duration]** The entire installation process (including CRD establishment and environment-specific resource application) typically takes approximately 3 minutes to complete.
- **[Troubleshooting]** Some users on specific macOS or Windows environments may encounter installation issues.
    - **Solution**: Delete the pods created during the failed attempt and use the platform-agnostic command instead of the environment-specific one.

Instead of using the environment-specific command:

```bash
kubectl apply -k github.com/kubeflow/pipelines/manifests/customize/env/dev?ref=$PIPELINE_VERSION
```

Use the platform-agnostic command:

```bash
kubectl apply -k github.com/kubeflow/pipelines/manifests/customize/env/platform-agnostic?ref=$PIPELINE_VERSION
```

### Verifying KFP Installation

- **[Pod Status]** After running the installation, the status of the pods in the `kubeflow` namespace can be checked using:

```bash
kubectl get pods -n kubeflow
```

    - In the provided example, pods are seen in various states such as `ContainerCreating` or `Running`.
- **[Underlying Engine]** Kubeflow Pipelines uses **Argo Workflows** as its underlying CI/CD orchestration engine.
    - This is identifiable by the presence of the `workflow-controller` among the running pods.

### Accessing the KFP Dashboard

- **[Port Forwarding]** Since the dashboard is running inside the Kubernetes cluster, it must be port-forwarded to be accessible via a local web browser.
- **[Execution]** To access the UI, the `ml-pipeline-ui` service is port-forwarded to a local port (e.g., 8080):

```bash
kubectl port-forward -n kubeflow svc/ml-pipeline-ui 8080:8080
```

    - Once the port forwarding is active, the dashboard can be accessed at `localhost:8080`.

### Writing Kubeflow Pipelines

- **[The Core Approach]** Pipelines are not written directly in YAML
    - Developers use the same Python programs they use for model development
    - This is achieved using a **DSL (Domain Specific Language)**
- **[Compilation Process]** The Python code is compiled into a YAML file
    - This abstraction makes the process simple because the user doesn't have to manually manage the complex YAML structure

### Using the Domain Specific Language (DSL)

- **[The Role of DSL]** It provides the necessary abstractions to transform standard Python code into a pipeline-ready format
    - ML engineers write standard Python scripts
    - They incorporate DSL-specific elements to define the pipeline structure:
        - Pre-built functions
        - Annotations
        - Decorators
        - Input and Output definitions
- **[The Workflow]** The process follows a clear progression from code to execution:

```mermaid
flowchart LR
    A["Python Script\n(with DSL)"] --> B["Compilation"]
    B --> C["YAML File"]
    C --> D["Upload to KFP Dashboard"]
```

- **[Deployment]** Once the Python code is compiled into a YAML file, it is uploaded directly to the Kubeflow Pipelines dashboard to be executed as a pipeline.

### Using the Kubeflow Pipelines Dashboard

- **[Uploading Pipelines]** Once a Python script is compiled into a YAML file, it can be uploaded to the dashboard to create a new pipeline
    - In the UI, select **Create a new pipeline**
    - Provide a name for the pipeline
    - Use the **Upload a file** option to select the compiled YAML file

### Setting Up the Python Environment

- **[Local Workspace Setup]** To begin writing the 'Hello World' pipeline, a dedicated directory is created in the terminal
...
- **[Why use them?]** To prevent dependency conflicts
    - When working on multiple projects (e.g., a DevOps script and a Machine Learning project) on the same machine, they may require different versions of the same common dependency
    - Virtual environments isolate these dependencies so they don't clash
- **[Implementation]** Creating and activating a hidden environment
    - It is a best practice to create the environment as a hidden folder (starting with a dot) to keep the workspace clean and simplify `.gitignore` management

```bash

# Create a hidden virtual environment named '.kfp'
python3 -m venv .kfp
...
- **[Isolation]** Packages installed inside the virtual environment are restricted to that specific environment
    - Running `deactivate` exits the environment
    - Once deactivated, the packages installed within it are no longer available on the global machine

### Installing the KFP SDK

- **[Installation]** The KFP package is installed via `pip` within the activated environment
- **[Version Pinning]** It is recommended to pin a specific version of the SDK to ensure compatibility, especially when using older Python versions (3.8 or 3.9)

```bash

# Recommended installation with version pinning
pip3 install kfp==2.9.0
```

- **[Purpose]** The KFP SDK is the package used to write the Python programs that define and create pipelines.

### Developing the First Pipeline

- **[The Python Script]** The actual pipeline logic is written in a standard Python file (e.g., `hello_pipeline.py`)
- **[DSL Integration]** The script uses the KFP SDK to incorporate Domain Specific Language (DSL) elements to define the pipeline structure, which is then compiled into a YAML file for the dashboard.

### Defining Pipeline Components with DSL

- **[Concept]** In Kubeflow, different stages of the Machine Learning lifecycle are treated as individual \*\*components"
    - Examples include data collection/loading, model training, and model evaluation
- **[Implementation]** ML engineers write standard Python functions for these stages and then apply a DSL decorator to make them compatible with Kubeflow
    - The `@dsl.component` decorator tells Kubeflow that a specific Python function should be treated as a reusable component within a pipeline
...
- **[Execution Model]** When a function is decorated as a component, Kubeflow handles its execution through Kubernetes pods
    - **Scalability & Parallelism**: Each component runs in its own independent pod, allowing different stages of the pipeline to scale or run in parallel without interfering with one another.

```python

# Define the pipeline using the @dsl.pipeline decorator
@dsl.pipeline(name="hello-world-pipeline", description="A basic pipeline that prints a greeting.")
...
- **[The Orchestrator]** The `@dsl.pipeline` decorator serves as the central orchestrator for the entire machine learning workflow
    - It manages the execution of all individual components, similar to how Jenkins or GitHub Actions orchestrate software tasks
- **[Real-World Context]** In a practical ML pipeline, the orchestration might involve a model designed to classify specific data types
    - **Example**: A flower classification model that identifies whether a specimen is a \\textit{Setosa}, \\textit{Versicolor}, or \\textit{Virginica} species.

### Real-World Example: Iris Flower Classification

- **[The Dataset]** The pipeline uses the Iris dataset, a standard public dataset for classification tasks.
    - **Input Features**: The model takes four specific measurements as input:
        - Sepal length
        - Sepal width
        - Petal length
        - Petal width
    - **Goal**: Based on these features, the model identifies the flower species: *Setosa*, *Versicolor*, or *Virginica*.
- **[Pipeline Stages]** An ML engineer breaks the workflow into discrete, decorated functions:
    - **Step 1: Data Loading**: A function to load the Iris dataset into memory.
    - **Step 2: Model Training**: A function to train the model using the loaded data.
    - **Orchestration**: The `@dsl.pipeline` decorator manages the execution and dependencies between these stages.
- **[Managing Dependencies]** Because each component executes in an independent Kubernetes pod, dependencies are not inherited from the local machine.
    - **[Requirement]** You must explicitly define required libraries in the `@dsl.component` decorator using `packages_to_install`.

```python

# Example of a component with explicit dependencies
@dsl.component(
...
### Completing the Iris Pipeline

- **[Managing Dependencies]** Every stage of the ML workflow requires its specific environment setup
    - For the second stage (model training), the `@dsl.component` decorator must include all necessary libraries, such as `scikit-learn`
    - **Tip**: In a professional setting, you can coordinate with developers to identify exactly which packages are required for a specific stage to ensure the container image has everything it needs to execute
- **[Pipeline Orchestration]** The `@dsl.pipeline` decorator is used where the individual component functions are invoked
    - It acts as the glue that connects `load_data()` and `train_model()`
    - You can optionally provide a `name` and a `description` for the pipeline to make it more readable in the dashboard
- **[The Compilation Step]** The final stage of the development process is converting the Python code into a format Kubeflow understands
    - The Python program must be compiled into a `.yaml` file
    - This YAML file is the actual artifact that is uploaded to the Kubeflow Pipelines dashboard to be executed on the Kubernetes cluster

```python

# Finalizing the pipeline orchestration and compilation logic
@dsl.pipeline(
...
# Step 4: Compile the pipeline to a YAML file
if __name__ == "__main__":
...
### Executing a 'Hello World' Pipeline

- **[Minimal Requirements]** To build a functional pipeline, you only need two primary imports from the KFP SDK:
    - `from kfp import dsl`: Used to define components and the pipeline structure
    - `from kfp import compiler`: Used to transform the Python code into a YAML file
- **[Standard Compilation Pattern]** The logic for compiling a pipeline into a YAML file is a constant pattern that does not change regardless of the pipeline's complexity

```python

# Standard pattern for compiling a pipeline
if __name__ == "__main__":
    kfp.compiler.Compiler().compile(
...
- **[Practical Implementation Steps]**
    ...
    1. Define the component using `@dsl.component`
    2. Define the pipeline using `@dsl.pipeline`
    3. Use the `kfp.compiler.Compiler().compile()` method within an `if __name__ == "__main__":` block
    4. Execute the script in the terminal (e.g., `python3 hello_pipeline.py`) to generate the `.yaml` artifact

### Uploading and Visualizing the Pipeline

- **[The YAML Artifact]** The compilation process automatically generates a `.yaml` file (e.g., `hello_world_pipeline.yaml`)
    - This file contains the logic for all steps that must be executed within Kubernetes pods
    - **[Why CRDs matter]** Kubernetes uses Custom Resource Definitions (CRDs) to understand this specific YAML structure, as it defines a resource type that is not part of standard Kubernetes
- **[Using the Dashboard]** Once the YAML file is generated, it is uploaded to the Kubeflow Pipelines dashboard to initiate a run
- **[Visualizing with a DAG]** After uploading and creating a run, the dashboard generates a Directed Acyclic Graph (DAG)
    - The DAG provides a visual representation of the pipeline's execution flow
    - It allows users to monitor the state of each step in the execution

### Executing a Pipeline Run

- **[Configuring Parameters]** When creating a run in the dashboard, you can specify custom parameters to pass into your Python functions
    - For example, passing a name like "Abhishek" to a component can change the output to "hello Abhishek"
    - **[Tip for Beginners]** It is recommended to leave advanced options like Service Accounts at their default settings when starting out
- **[Monitoring Execution via Kubernetes]** While the dashboard shows a "pending execution" status, the actual work is being orchestrated by Kubernetes pods
    - You can verify this by running `kubectl get pods -n kubeflow` in your terminal
    - **[The Pod Lifecycle during a Run]**

        1. **Initialization**: The system first initializes necessary drivers (e.g., the Argo driver)
        2. **Implementation**: Once drivers are ready, Kubernetes creates new "implementation pods"
        3. **Execution**: These implementation pods are what actually run the logic defined in your pipeline components

```bash

# Check the status of pods in the kubeflow namespace
kubectl get pods -n kubeflow
```

| Pod Status | Description |
| --- | --- |
| Pending | The system is initializing drivers or waiting for resources |
| Running | The implementation pod is actively executing the pipeline step |
| Succeeded/Failed | The specific component task has completed |

### Monitoring Pipeline Completion

- **[Tracking Pod Transitions]** During execution, the implementation pod moves through various states in the terminal:
    - **Init**: The pod is initializing or getting ready
    - **Running**: The component is actively executing
    - **Completed/Successful**: The task has finished successfully, indicated by a checkmark or success symbol in the dashboard
- **[Inspecting Component Results]** Once a component in the DAG shows a successful execution, you can click on it to drill down into the details:
    - **Inputs/Outputs**: View the data passed into and out of the component
    - **Logs**: Access the stdout/stderr to verify the actual logic. For example, in the "say-hello" component, the logs will confirm the output: `hello Abhishek`

### Initiating New Pipeline Runs

- To run a different version of a pipeline or change parameters, you can use the "Start a new run" feature
    - This allows you to select from existing compiled pipelines (e.g., choosing between different versions of the "hello" pipeline) to test different configurations or logic.

### Pod Management During Pipeline Runs

- **[New Pod Creation]** Every time a new pipeline run is initiated, the system creates a new set of pods in the Kubernetes namespace:
    - A new **Argo driver pod** is created to manage the workflow
    - New **implementation pods** are created to execute the specific component logic
- **[Stale Pods]** Running multiple pipelines can result in many pods being present in the cluster
    - These can be managed or removed within Argo workflows to keep the environment clean

### Transitioning to Machine Learning Pipelines

- **[From 'Hello World' to ML]** The initial 'Hello World' pipeline serves as a conceptual verification that the Kubeflow orchestration and Kubernetes integration are working correctly
- **[The Iris Classification Pipeline]** The next stage involves a functional Machine Learning pipeline designed to classify flowers based on specific features
    - **Input Features**: The model takes four specific measurements as input to make a prediction

```python

# Example of the Iris pipeline structure being prepared
import kfp
import dsl
...
### The Iris Classification Pipeline

- **[The Dataset]** A widely used dataset in machine learning for classification tasks
    - It consists of four input features: sepal length, sepal width, petal length, and petal width
    - The goal is to predict which of three species the flower belongs to: Setosa, Versicolor, or Virginica
- **[Pipeline Logic]** An ML engineering team approaches this by defining distinct stages that are then invoked in a specific order:
    - **Step 1**: Load the data into memory
    - **Step 2**: Train the model using the loaded data
- **[Implementation Requirements]** To convert these manual ML stages into a Kubeflow workflow, specific libraries must be imported:
    - `kfp`: The Kubeflow Pipelines SDK
    - `dsl`: The Domain Specific Language for defining components and orchestration
    - `compiler`: Used to compile the Python code into a YAML file for execution
...
### Iris Pipeline Implementation Details

- **[Component 1: Data Loading]** The `load_data` component is configured to run in a containerized environment:
    - **Base Image**: `python:3.8-slim`
    - **Required Packages**: `pandas` and `scikit-learn` (necessary for reading the dataset)
- **[Component 2: Model Training]** The `train_model` component implements a standard machine learning training split:
    - **Algorithm**: Uses a `RandomForestClassifier`
    - **Data Split**: 80% of the data is used for training, while the remaining 20% is used to verify the model's accuracy
- **[Pipeline Compilation]** Once the components and orchestration are defined, the Python script must be compiled into a format Kubernetes can understand:
    - **Tool**: `kfp.compiler.Compiler().compile()`
    - **Output**: A YAML file (e.g., `iris_pipeline.yaml`) that represents the pipeline's structure and execution steps
...
- **[Compilation]** The Python script is executed to generate the deployment artifact:
    - Command: `python3 iris-pipeline.py`
    - Result: Creates `iris_pipeline.yaml`
- **[Dashboard Upload]** The YAML file is used to register the pipeline in the Kubeflow UI:
    - **Pipeline Name**: `flower species identifier`
    - **File**: `iris_pipeline.yaml`
- **[Pipeline Structure]** Upon uploading, the dashboard visualizes the pipeline as a Directed Acyclic Graph (DAG) containing two components:

    1. `load_data` component
    2. `train_model` component

- **[Execution]** A new run is initiated by clicking the **Start** button with default options.
- **[Monitoring via Kubernetes]** The underlying activity can be tracked using the terminal to observe the lifecycle of the pods created for the run:
    - Command: `kubectl get pods -n kubeflow`
    - Pods will transition through various states (e.g., `Pending`, `Running`, `Completed`) as the pipeline stages execute.

### Monitoring Pipeline Execution

- **[Kubernetes Pod Lifecycle]** Using `kubectl` to track the real-time status of the components being orchestrated:
    - **DAG Driver**: The first pod created to manage the execution flow (e.g., `iris-no-artifacts-pipeline-2n7kp-system-container-driver`).
    - **Implementation Pods**: Individual pods created to execute specific pipeline stages (e.g., `load_data`).
    - **Pod States**: Pods transition through various statuses during a run:
        - `Pending`: The pod is being scheduled.
        - `PodInitializing`: The container is starting up.
        - `Running`: The component is actively executing its logic.
        - `Succeeded` / `Completed`: The task finished successfully.
        - `Failed`: The task encountered an error.
- **[Verifying Component Output]** Once a component (like `load_data`) reaches the `Completed` state, its logs can be inspected in the Kubeflow dashboard to ensure the data was processed correctly.
    - **Example Output**: For the Iris dataset `load_data` component, the logs confirm the successful generation of 4 input parameters:
        - `sepal_length`
        - `sepal_width`
        - `petal_length`
        - `petal_width`
- **[Dataset Status]** The `load_data` component has finished execution, indicating the dataset is completely loaded and ready for subsequent stages of the pipeline.

### Pipeline Results and Value

- **[Model Performance]** The `train_model` component has completed execution, yielding a specific performance metric:
    - **Accuracy**: 0.9
- **[The Value of Orchestration]** The primary goal of creating this pipeline is to reduce the manual workload for machine learning engineers.
    - **Automated Iteration**: If the underlying data changes (e.g., moving from the Iris dataset to a different one) or if new features are added to the existing dataset, the engineer does not need to manually trigger each stage.
    - **Seamless Updates**: By simply modifying the Python function responsible for data loading, the entire downstream process—loading, training, and evaluation—can be re-run automatically through the pipeline.